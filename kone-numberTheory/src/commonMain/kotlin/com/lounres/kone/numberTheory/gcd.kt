 ...
 */
public data class BezoutIdentityWithGCD<T>(val first: T, val second: T, val gcd: T)
 ...
 * @usesMathJax
 */
public tailrec fun gcd(a: Int, b: Int): Int = if (a == 0) abs(b) else gcd(b % a, b)

/**
 * Computes [Greatest Common Divisor](https://en.wikipedia.org/wiki/Greatest_common_divisor) of the [values].
 */
public fun gcd(vararg values: Int): Int = values.reduce(::gcd)
public fun gcd(values: Iterable<Int>): Int = values.reduce(::gcd)
 ...
 */
public fun bezoutIdentityWithGCD(a: Int, b: Int): BezoutIdentityWithGCD<Int> =
    when {
 ...
 * @usesMathJax
 */
public tailrec fun gcd(a: Long, b: Long): Long = if (a == 0L) abs(b) else gcd(b % a, b)

/**
 * Computes [Greatest Common Divisor](https://en.wikipedia.org/wiki/Greatest_common_divisor) of the [values].
 */
public fun gcd(vararg values: Long): Long = values.reduce(::gcd)
public fun gcd(values: Iterable<Long>): Long = values.reduce(::gcd)

/**
 * Computes "the smallest" [Bézout coefficients](https://en.wikipedia.org/wiki/B%C3%A9zout%27s_identity) and
 * [GCD](https://en.wikipedia.org/wiki/Greatest_common_divisor) of [a] and [b].
 */
public fun bezoutIdentityWithGCD(a: Long, b: Long): BezoutIdentityWithGCD<Long> =
    when {
        a < 0 && b < 0 -> with(bezoutIdentityWithGCDInternalLogic(-a, -b, 1, 0, 0, 1)) { BezoutIdentityWithGCD(-first, -second, gcd) }
        a < 0 -> with(bezoutIdentityWithGCDInternalLogic(-a, b, 1, 0, 0, 1)) { BezoutIdentityWithGCD(-first, second, gcd) }
        b < 0 -> with(bezoutIdentityWithGCDInternalLogic(a, -b, 1, 0, 0, 1)) { BezoutIdentityWithGCD(first, -second, gcd) }
 ...
 * Also assumes that [a] and [b] are non-negative. TODO: Docs
 */
internal tailrec fun bezoutIdentityWithGCDInternalLogic(a: Long, b: Long, m1: Long, m2: Long, m3: Long, m4: Long): BezoutIdentityWithGCD<Long> =
    if (b == 0L) BezoutIdentityWithGCD(m1, m3, a)
    else {
        val quotient = a / b
        val reminder = a % b
        bezoutIdentityWithGCDInternalLogic(b, reminder, m2, m1 - quotient * m2, m4, m3 - quotient * m4)
