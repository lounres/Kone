...
In a polynomial space over ring `R` we specify algebraic operations on polynomials, and algebraic operations on polynomials and constants. Basically, the polynomial main interfaces look like this (see API reference for [`PolynomialSpace`](https://lounres.github.io/Kone/api//libs/main/polynomial/com.lounres.kone.polynomial/-polynomial-space/index.html) and [`MultivariatePolynomialSpace`](https://lounres.github.io/Kone/api/libs/main/polynomial/com.lounres.kone.polynomial/-multivariate-polynomial-space/index.html)):
```
...
There are also division operations in case of a field `R`. See [`PolynomialSpaceOverField`](https://lounres.github.io/Kone/api/api/libs/main/polynomial/com.lounres.kone.polynomial/-polynomial-space-over-field/index.html) and [`MultivariatePolynomialSpaceOverField`](https://lounres.github.io/Kone/api/api/libs/main/polynomial/com.lounres.kone.polynomial/-multivariate-polynomial-space-over-field/index.html)

### Implementations of polynomial spaces

In Kone, there are two types of polynomials:
1. [`ListPolynomial`](https://lounres.github.io/Kone/api/api/libs/main/polynomial/com.lounres.kone.polynomial/-list-polynomial/index.html) that represents a univariate polynomial $a_0 + \dots + a_n x^n$ as a list $[a_0, \dots, a_n]$. Thus, `ListPolynomial<E>` just wraps `List<E>`.
2. [`LabelledPolynomial`](https://lounres.github.io/Kone/api/api/libs/main/polynomial/com.lounres.kone.polynomial/-labeled-polynomial/index.html) that represents a multivariate polynomial $\sum_{i=1}^n a_i x_1^{d_{1, i}} \dots x_k^{d_{k, i}}$ as a map $\{s_1 \to a_1, \dots, s_n \to a_n\}$ where each $s_i$ is a signature of $a_i x_1^{d_{1, i}} \dots x_k^{d_{k, i}}$ and is represented as a map $\{x_1 \to d_{1, i}, \dots, x_k \to d_{k, i}\}$. Thus, `LabelledPolynomial<E>` just wraps `Map<Map<Symbol, UInt>, E>`.

:::caution
`NumberedPolynomial` implementation is deprecated and is going to be removed soon.
:::

Both types of polynomials have associated polynomial spaces ([`ListPolynomialSpace`](https://lounres.github.io/Kone/api/libs/main/polynomial/com.lounres.kone.polynomial/-list-polynomial-space/index.html) and [`LabelledPolynomial`](https://lounres.github.io/Kone/api/libs/main/polynomial/com.lounres.kone.polynomial/-labeled-polynomial-space/index.html)) that are constructed on provided ring.

### Rational functions spaces interfaces

In a rational functions space over ring `R` we specify algebraic operations on constants, polynomials, (variables,) and rational functions. Basically, the polynomial main interfaces look like this (see API reference for [`RationalFunctionSpace`](https://lounres.github.io/Kone/api/libs/main/polynomial/com.lounres.kone.polynomial/-rational-function-space/index.html) and [`RationalFunctionSpaceOverField`](https://lounres.github.io/Kone/api/libs/main/polynomial/com.lounres.kone.polynomial/-rational-function-space-over-field/index.html)):
```
// A bit simplified. See full API in API reference.
...
In Kone, there are two types of rational functions: [`ListRationalFunction`](https://lounres.github.io/Kone/api/libs/main/polynomial/com.lounres.kone.polynomial/-list-rational-function/index.html) and [`LabeledRationalFunction`](https://lounres.github.io/Kone/api/libs/main/polynomial/com.lounres.kone.polynomial/-labeled-rational-function/index.html). They both are just represented as fractions of corresponding polynomial types. Both types of rational functions have associated spaces ([`ListRationalFunctionSpace`](https://lounres.github.io/Kone/api/libs/main/polynomial/com.lounres.kone.polynomial/-list-rational-function-space/index.html) and [`LabeledRationalFunctionSpace`](https://lounres.github.io/Kone/api/libs/main/polynomial/com.lounres.kone.polynomial/-labeled-rational-function-space/index.html)) that are constructed on provided ring.
