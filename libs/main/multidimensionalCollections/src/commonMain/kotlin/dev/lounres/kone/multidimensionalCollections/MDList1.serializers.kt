            ...
            if (decodeSequentially()) { // sequential decoding protocol
                ...
                while (true) {
                    when (val index = decodeElementIndex(descriptor)) {
                        ...
                        else -> error("Cannot deserialize MDList1: got index $index out of range [0; 2)")
                    }
                }
                shape = shapeBuilder ?: error("Cannot deserialize MDList1: did not receive element with index 0")
                content = contentBuilder ?: error("Cannot deserialize MDList1: did not receive element with index 1")
            }
            check(shape.size == 1u) { "Cannot deserialize MDList1: received shape and content size mismatch" }
            check(content.size == shape[0u]) { "Cannot deserialize MDList1: received shape and content size mismatch" }
            ArrayMDList1(size = shape[0u], data = content)
        }
}
