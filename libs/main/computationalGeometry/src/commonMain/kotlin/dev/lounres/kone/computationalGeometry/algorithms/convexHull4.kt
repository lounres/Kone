...
internal fun <
    Number,
    Polytope: PolytopicConstruction4Polytope<Number, Polytope, Vertex>,
    Vertex: PolytopicConstruction4Vertex<Number, Polytope, Vertex>,
> giftWrapping4Atom(
    startPoint: Point4<Number>,
    normalGiftWrapping4Vector: Vector4<Number>,
    tangentGiftWrapping4Vector: Vector4<Number>,
    ...
    return otherPoints.minListWithBy(
        ...
        TangentFraction(v dot tangentGiftWrapping4Vector, v dot normalGiftWrapping4Vector)
    }
}
...
internal fun <
    ...
    Polytope: PolytopicConstruction4Polytope<Number, Polytope, Vertex>,
    Vertex: PolytopicConstruction4Vertex<Number, Polytope, Vertex>,
> ExtendablePolytopicConstruction4<Number, Polytope, Vertex>.giftWrapping4Increment(
    ...
    while (facetsToProcess.isNotEmpty()) {
        ...
        for (subfacet in facet.facesOfDimension(subspaceDimension - 2u)) if (subfacet in subfacetsToProcess) {
            val startPoint: Point4<Number>
            val normalGiftWrapping4Vector: Vector4<Number>
            val tangentGiftWrapping4Vector: Vector4<Number>
            scope {
                ...
                tangentGiftWrapping4Vector = orthogonalizedBasis[subspaceDimension-2u]
                normalGiftWrapping4Vector = orthogonalizedBasis[subspaceDimension-1u]
            }

            val newVertices: KoneList<Vertex> = giftWrapping4Atom<Number, Polytope, Vertex>(
            ...
            val newFacet: Polytope = giftWrapping4Increment(
...
internal data class Wrapping4Result<Number, Polytope, Vertex>(
    var polytope: Polytope,
    val computedFacesRegistry: KoneMutableMap<KoneSet<Vertex>, Polytope>,
    val startPoint: Point4<Number>,
    val orthogonalizationState: GramSchmidtOrthogonalizationIntermediateState4<Number>,
)
...
internal fun <
    ...
    Polytope: PolytopicConstruction4Polytope<Number, Polytope, Vertex>,
    Vertex: PolytopicConstruction4Vertex<Number, Polytope, Vertex>,
> ExtendablePolytopicConstruction4<Number, Polytope, Vertex>.giftWrapping4Extension(
    vertexReification: Reification<Vertex>,
    vertexEquality: Equality<Vertex>,
    vertexHashing: Hashing<Vertex>?,
    vertexOrder: Order<Vertex>?,
    ...
    polytopeOrder: Order<Polytope>?,
    subspaceDimension: UInt,
    wrappingResult: Wrapping4Result<Number, Polytope, Vertex>,
    normalVector: Vector4<Number>,
    otherPoints: KoneIterable<Vertex>,
) {
    ...
    while (otherPoints.isNotEmpty()) {
        if (wrappingResult.orthogonalizationState.orthogonalizedBasis.size == subspaceDimension - 1u) {
            val resultingPolytope = giftWrapping4Increment(
                vertexReification = vertexReification,
                vertexEquality = vertexEquality,
                vertexHashing = vertexHashing,
                vertexOrder = vertexOrder,
        ...
        val nextPoints = giftWrapping4Atom(
            startPoint = wrappingResult.startPoint,
            normalGiftWrapping4Vector = currentNormalVector,
            tangentGiftWrapping4Vector = tangentVector,
            otherPoints = otherPoints
        )

        giftWrapping4Extension(
            vertexReification = vertexReification,
            vertexEquality = vertexEquality,
            vertexHashing = vertexHashing,
            vertexOrder = vertexOrder,
...
internal fun <
    ...
    Polytope: PolytopicConstruction4Polytope<Number, Polytope, Vertex>,
    Vertex: PolytopicConstruction4Vertex<Number, Polytope, Vertex>,
> ExtendablePolytopicConstruction4<Number, Polytope, Vertex>.giftWrapping4Full(
    vertexReification: Reification<Vertex>,
    vertexEquality: Equality<Vertex>,
    vertexHashing: Hashing<Vertex>?,
    vertexOrder: Order<Vertex>?,
    ...
    if (subspaceDimension == 0u) {
        ...
        return Wrapping4Result(
            ...
            orthogonalizationState = GramSchmidtOrthogonalizationIntermediateState4(
                orthogonalizedBasis = KoneArrayFixedCapacityList(4u),
                product = one,
                exclusiveProducts = KoneArrayFixedCapacityList(4u)
            )
        )
    }

    val startPoints = points.minListBy { it.position.coordinates[subspaceDimension - 1u] }
    val wrappingResult = giftWrapping4Full(
        vertexReification = vertexReification,
        vertexEquality = vertexEquality,
        vertexHashing = vertexHashing,
        vertexOrder = vertexOrder,
    ...
    giftWrapping4Extension(
        ...
        subspaceDimension = subspaceDimension,
        wrappingResult = wrappingResult,
        normalVector = Vector4(ColumnVector(4u) { if (it == subspaceDimension - 1u) one else zero }),
...
public fun <
    ...
    Polytope: PolytopicConstruction4Polytope<Number, Polytope, Vertex>,
    Vertex: PolytopicConstruction4Vertex<Number, Polytope, Vertex>,
> ExtendablePolytopicConstruction4<Number, Polytope, Vertex>.constructConvexHullByGiftWrapping4(
    vertexReification: Reification<Vertex>,
    vertexEquality: Equality<Vertex>,
    vertexHashing: Hashing<Vertex>?,
    vertexOrder: Order<Vertex>?,
    ...
    return giftWrapping4Full(
        ...
        polytopeHashing = polytopeHashing,
        polytopeOrder = polytopeOrder,
        subspaceDimension = 4u,
        points = vertices,
    ).polytope
}

context(koneContextRegistry: KoneContextRegistry, _: Ring<Number>, _: Order<Number>, _: EuclideanKategory4<Number>)
public fun <
    Number,
    Polytope: PolytopicConstruction4Polytope<Number, Polytope, Vertex>,
    Vertex: PolytopicConstruction4Vertex<Number, Polytope, Vertex>,
