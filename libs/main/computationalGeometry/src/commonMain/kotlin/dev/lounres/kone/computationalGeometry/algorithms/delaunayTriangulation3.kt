...
> ExtendablePolytopicConstruction3<Number, Polytope, Vertex>.constructDelaunayTriangulation3(
    ...
    val paraboloidDimension = 4u
    
    val paraboloidPolytopicConstruction = AbstractPolytopicConstruction4<Number>()
    
    val simplicesMapping = KoneMutableMap.of<AbstractPolytopicConstruction4Polytope<Number>, Polytope>(keyEquality = absoluteEquality(), keyHashing = defaultHashing())
    val verticesMapping = vertices.associateBy(
        keyEquality = absoluteEquality(),
        keyHashing = defaultHashing(),
    ) { oldVertex ->
        val newVertex = paraboloidPolytopicConstruction.addVertex(Point4(MDList1(paraboloidDimension) { if (it < paraboloidDimension - 1u) oldVertex.position.coordinates[it] else oldVertex.position.sumOf { c -> c * c } }))
        simplicesMapping[newVertex.asPolytope()] = oldVertex.asPolytope()
        newVertex
    }

    val convexHull: AbstractPolytopicConstruction4Polytope<Number> =
        paraboloidPolytopicConstruction.constructConvexHullByGiftWrapping4(
