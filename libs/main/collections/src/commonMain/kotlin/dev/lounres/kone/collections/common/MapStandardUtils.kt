...
public fun <K, V> KoneMutableMap<in K, V>.getOrNullAndRemove(key: K): V? = getMaybeAndRemove(key).orDefault(null)
...
public operator fun <K, V> KoneMap<out K, V>.iterator(): KoneIterator<KoneMapEntry<K, V>> = entries.iterator()