...
public fun <K, V1: W, V2: W, W> merge(map1: KoneMap<out K, V1>, map2: KoneMap<out K, V2>): KoneMap<K, W> {
    val result = KoneHashResizableMap<K, W>() // TODO: Replace with growable map
...
public inline fun <T, K, V, D : KoneMutableMap<K, V>> KoneIterable<T>.associateByTo(destination: D, keySelector: (T) -> K, valueTransform: (T) -> V, resolve: (key: K, currentValue: V, newValue: V) -> V): D {
    for (element in this) {
...
public inline fun <T, K, V> KoneIterable<T>.associate(transform: (T) -> Pair<K, V>, resolve: (key: K, currentValue: V, newValue: V) -> V): KoneMap<K, V> =
    associateTo(KoneHashResizableMap<K, V>(), transform, resolve)

public inline fun <T, K, V> KoneIterable<T>.associateBy(keySelector: (T) -> K, valueTransform: (T) -> V, resolve: (key: K, currentValue: V, newValue: V) -> V): KoneMap<K, V> =
    associateByTo(KoneHashResizableMap(), keySelector, valueTransform, resolve)

public inline fun <T, K> KoneIterable<T>.associateBy(keySelector: (T) -> K, resolve: (key: K, currentValue: T, newValue: T) -> T): KoneMap<K, T> =
    associateByTo(KoneHashResizableMap(), keySelector, resolve)

public inline fun <K, V, W, D : KoneMutableMap<K, W>> KoneMap<out K, V>.mapValuesTo(destination: D, transform: (KoneMapEntry<K, V>) -> W, resolve: (key: K, currentValue: W, newValue: W) -> W): D =
    entriesView.associateByTo(destination, { it.key }, transform, resolve)

public inline fun <K, V, L, D : KoneMutableMap<L, V>> KoneMap<out K, V>.mapKeysTo(destination: D, transform: (KoneMapEntry<K, V>) -> L, resolve: (key: L, currentValue: V, newValue: V) -> V): D =
    entriesView.associateByTo(destination, transform, { it.value }, resolve)

public inline fun <K, V, L> KoneMap<out K, V>.mapKeys(transform: (KoneMapEntry<K, V>) -> L, resolve: (key: L, currentValue: V, newValue: V) -> V): KoneMap<L, V> =
    mapKeysTo(KoneHashResizableMap(), transform, resolve)
