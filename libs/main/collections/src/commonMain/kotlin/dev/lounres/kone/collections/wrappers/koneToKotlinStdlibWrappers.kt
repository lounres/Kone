...
// region Maps
internal class KotlinStdlibWrapperKoneMapEntriesKoneIteratorAsMapEntriesIterator<out K, out V>(private val iterator: KoneIterator<KoneMap.Entry<K, V>>): Iterator<Map.Entry<K, V>> {
    override fun hasNext(): Boolean = iterator.hasNext()
...
internal class KoneWrapperMapEntryIterator<out K, out V>(private val iterator: Iterator<Map.Entry<K, V>>): KoneIterator<KoneMap.Entry<K, V>> {
    var holder: @UnsafeVariance KoneMap.Entry<@UnsafeVariance K, @UnsafeVariance V>? = null
...
internal class KoneMappingWrapperMapEntryCollection<out K, out V>(private val collection: Collection<Map.Entry<K, V>>): KoneIterableCollection<KoneMap.Entry<K, V>> {
    override fun toString(): String = "KoneWrapperIterableCollection($collection)"

    override val size: UInt get() = collection.size.toUInt()

    override fun contains(element: KoneMap.Entry<@UnsafeVariance K, @UnsafeVariance V>): Boolean =
        collection.contains(
            object : Map.Entry<K, V> {
    ...
    override fun iterator(): KoneIterator<KoneMap.Entry<K, V>> = KoneWrapperMapEntryIterator(collection.iterator())
}

internal class KotlinStdlibWrapperKoneMapEntries<K, V>(private val entries: KoneIterableSet<KoneMap.Entry<K, V>>): Set<Map.Entry<K, V>> {
    override val size: Int get() = entries.size.toInt()
    override fun isEmpty(): Boolean = entries.isEmpty()

    override fun contains(element: Map.Entry<K, V>): Boolean =
        entries.contains(KoneMap.Entry(element.key, element.value))
