...
public interface MultivariatePolynomialManipulationSpace<C, V, MS, MutMS: MS, P: Polynomial<C>, MutP: P, out A: Ring<C>>: MultivariatePolynomialSpace<C, V, P, A> {
    ...
    public fun monomialSignatureOf(vararg variablePowers: VariablePower<V>): MS = mutableMonomialSignatureOf(*variablePowers)
    public fun monomialSignatureOf(variablePowers: Collection<VariablePower<V>>): MS = mutableMonomialSignatureOf(variablePowers)
    public val MS.size: Int
    public fun MS.isEmpty(): Boolean = size == 0
    public fun MS.containsVariable(variable: V): Boolean = get(variable) != Option.None
    public operator fun MS.get(variable: V): Option<UInt>
    public fun MS.getOrDefault(variable: V): UInt = get(variable).getOrDefault(0u)
    ...
    public fun mutableMonomialSignatureOf(vararg variablePowers: VariablePower<V>): MutMS
    public fun mutableMonomialSignatureOf(variablePowers: Collection<VariablePower<V>>): MutMS
    ...
    public fun P.containsSignature(signature: MS): Boolean = get(signature) != Option.None
    public fun P.get(signature: MS): Option<C>
    public fun P.getOrDefault(signature: MS): C = get(signature).getOrDefault(constantZero)
    ...
    override val one: P get() = polynomialOf(Monomial(ring.one, monomialSignatureOf()))
    ...
    public override fun valueOf(value: C): P = polynomialOf(Monomial(value, monomialSignatureOf()))
