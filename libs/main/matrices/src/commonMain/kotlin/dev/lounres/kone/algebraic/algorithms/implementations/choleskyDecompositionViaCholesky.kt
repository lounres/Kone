...
private class CholeskyDecompositionViaCholesky<Number, Matrix : MDList2<Number>>(
    ...
    override fun Matrix.choleskyDecomposition(): CholeskyDecomposition<Number, Matrix> {
        ...
        for (k in 0u ..< n) {
            val aK = a[k, k]
            val aKSqrt = aK.positiveSquareRoot()
            
            for (i in k ..< n)
                l[i, k] = (k .. i).asKoneSequence().sumOf { l[i, it] * a[it, k] } / aKSqrt
...
public fun <Number, Matrix : MDList2<Number>> CholeskyDecompositionComputer.Companion.viaCholesky(
...
): CholeskyDecompositionComputer<Number, Matrix> = CholeskyDecompositionViaCholesky(
...
public object CholeskyDecompositionComputerCholeskySuppliableTopLevelFunctions {
    @Suppliable
    context(koneContextRegistry: KoneContextRegistry.Provider)
    public fun <@Supply Number, @Supply Matrix : MDList2<Number>> CholeskyDecompositionComputer.Companion.viaCholesky(): CholeskyDecompositionComputer<Number, Matrix> {
    ...
    public fun <@Supply Number, @Supply Matrix : MDList2<Number>> CholeskyDecompositionComputer.Companion.setViaCholesky(
        matrixFactory: MatrixFactory<Number, Matrix>,
        numberField: Field<Number>,
        positiveSquareRootComputer: PositiveSquareRootComputer<Number>,
        transposeMatrixComputer: TransposeMatrixComputer<Number, Matrix>,
    ...
    public fun <@Supply Number, @Supply Matrix : MDList2<Number>> CholeskyDecompositionComputer.Companion.useViaCholesky(
        ...
        CholeskyDecomposition.Key<Number, MatrixWithProperties<Number, Matrix>>() correspondsTo RegisteredValueProvider.cached {
            val choleskyDecompositionComputer = viaCholesky(
                matrixFactory = matrixFactory,
                numberField = numberField,
                positiveSquareRootComputer = positiveSquareRootComputer,
                transposeMatrixComputer = transposeMatrixComputer,
    ...
    public fun <@Supply Number, @Supply Matrix : MDList2<Number>> CholeskyDecompositionComputer.Companion.useViaCholesky() {
        CholeskyDecomposition.Key<Number, MatrixWithProperties<Number, Matrix>>() correspondsTo RegisteredValueProvider.cached {
            val choleskyDecompositionComputer = viaCholesky<Number, MatrixWithProperties<Number, Matrix>>()
            choleskyDecompositionComputer { matrix.get().choleskyDecomposition() }
        }
    }
}