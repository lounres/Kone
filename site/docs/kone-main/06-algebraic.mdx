...
So in Kone any ring is implemented as an object that holds all the operations of the ring, so it can be used as an algebraic context. For example, there `kone.numberTheory` provides class of Rational numbers and field context for them:
```kotlin
// Simplified definition. See actual API in API reference
data class Rational(val numerator: Long, val denumerator: Long)

public data object RationalField : Field<Rational> /* Field is an extension interface of Ring */ {
...
import dev.lounres.kone.numberTheory.*
